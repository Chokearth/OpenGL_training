...
int main() {
    ...
    // Create the camera
    Camera camera(width, height, glm::vec3(.0f, 1.0f, 2.0f));

    // Load the model
     glm::mat4 rot = glm::mat4_cast(glm::quat(.0f, 1.0f, .0f, .0f));
     glm::mat4 scale = glm::scale(glm::mat4(1.0f), glm::vec3(5.0f));
     Model model("res/models/scene/scene.gltf", rot * scale);

    while (!glfwWindowShouldClose(window)) {
        // Clear background
        glClearColor(.85f, .85f, .90f, 1.f);
