...
//    glfwSwapInterval(0);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // Enable modifying the stencil buffer
        glStencilMask(0xFF);

        // Draw the model
        model.draw(shaderProgram, camera);

        // Make the test pass only if the pixels is not 1
        glStencilFunc(GL_NOTEQUAL, 1, 0xFF);
        // Disable modifying the stencil buffer
        glStencilMask(0x00);
        ...
        glUniform1f(glGetUniformLocation(outliningProgram.ID, "outlining"), 0.03f);
